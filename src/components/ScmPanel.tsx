...
export default function ScmPanel({ data }: ScmPanelProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7, duration: 0.6 }} className="glass-panel p-3 pointer-events-auto mt-3 border border-[#D0D0D0]/30" style={{ background: 'rgba(208, 208, 208, 0.05)' }}>
      <button onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2">
        <div className="flex items-center gap-2">
          <Target className="w-3.5 h-3.5 text-[#D0D0D0]" />
        ...
        {expanded && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden flex flex-col flex-1 min-h-0">
            <div className="space-y-3 mt-2 overflow-y-auto styled-scrollbar pr-1 flex-1 pb-4">
              ...
              {marketAlerts.length > 0 && (
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <AlertCircle className="w-3 h-3 text-[#E0E0E0]" />
                    <span className="text-[9px] font-mono text-[#E0E0E0] tracking-widest font-bold">MARKET IMPACT ALERTS</span>
                  </div>
                  <div className="space-y-1">
                    {marketAlerts.map((alert: string, i: number) => (
                      <div key={i} className="px-2 py-1.5 rounded border border-[#E0E0E0] bg-[#E0E0E0]/10 text-[#E0E0E0] text-[9px] font-mono leading-tight shadow-[0_0_8px_rgba(224,224,224,0.15)]">
                        {alert}
                      </div>
                    ))}
                  </div>
                    ...
                    {criticalSuppliers.map((s: any, i: number) => {
                      ...
                      return (
                        <div key={i} className="px-2 py-1.5 rounded border border-[#FF1744]/40 bg-[#FF1744]/10">
                          ...
                          <div className="text-[8px] font-mono text-[#E0E0E0]">{threats.join(' • ')}</div>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1.5">
                  <Anchor className="w-3 h-3 text-[#D0D0D0]" />
                ...
                {(congestedPorts.length === 0 && riskyChokes.length === 0) ? (
                  ...
                  <div className="space-y-1">
                    {riskyChokes.map((c: any, i: number) => (
                      <div key={`c-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: c.risk === 'CRITICAL' ? '#FF1744' : '#D0D0D0' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          <span className="text-[10px] font-mono text-[#D0D0D0] font-bold">{c.name}</span>
                          <span className="text-[8px] font-mono font-bold px-1 rounded" style={{ background: c.risk === 'CRITICAL' ? '#FF1744' : '#D0D0D0', color: '#000' }}>{c.risk}</span>
                        </div>
                        <div className="text-[8px] font-mono text-[#aaa]">{c.traffic}</div>
                      </div>
                    ))}
                    {congestedPorts.map((p: any, i: number) => (
                      <div key={`p-${i}`} className="px-2 py-1.5 rounded hover:bg-white/5 transition-colors border-l-2" style={{ borderLeftColor: p.congestion === 'SEVERE' ? '#FF1744' : '#D0D0D0' }}>
                        <div className="flex justify-between items-center mb-0.5">
                          <span className="text-[10px] font-mono text-[#00BCD4]">{p.name}</span>
                          <span className="text-[8px] font-mono font-bold px-1 rounded" style={{ background: p.congestion === 'SEVERE' ? '#FF1744' : '#D0D0D0', color: '#000' }}>{p.congestion}</span>
                        </div>
