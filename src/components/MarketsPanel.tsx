  ...
  return (
    <div className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-[var(--hover-accent)] transition-colors">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`text-[9px] font-mono font-bold flex items-center gap-0.5 ${d.up ? 'text-[#E0E0E0]' : 'text-[var(--alert-red)]'}`}>
    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6, duration: 0.6 }} className={`glass-panel p-3 pointer-events-auto transition-all duration-300 flex flex-col ${maximized ? 'fixed inset-4 z-[9999] bg-[#0a0a09]/95 backdrop-blur-3xl' : ''}`}>
      <div onClick={() => setExpanded(!expanded)} className="flex items-center justify-between w-full mb-2 cursor-pointer">
        <div className="flex items-center gap-2">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">OPENSPY MARKETS</span>
          <span className="gotham-tag gotham-tag--low" style={{ fontSize: '7px', padding: '1px 4px' }}>LIVE</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full bg-[#E0E0E0] animate-openspy-pulse" />
            ...
            {markets.scm_alerts && markets.scm_alerts.length > 0 && (
              <div className="mb-2 space-y-1">
                {markets.scm_alerts.map((alert: string, i: number) => (
                  <div key={i} className="px-2 py-1.5 rounded border border-[#D0D0D0] bg-[#D0D0D0]/10 text-[#D0D0D0] text-[9px] font-mono leading-tight shadow-[0_0_8px_rgba(208,208,208,0.15)]">
                    {alert}
                  </div>
                ))}
              </div>
