...
function LayerPanel({ data, activeLayers, setActiveLayers, isMobile, theme = 'core', setTheme, isVisible }: LayerPanelProps) {
  ...
  return (
    ...
    <motion.div
      ...
      className="absolute top-1/2 -translate-y-1/2 left-0 w-[48px] flex flex-col items-center z-50 glass-panel p-2 rounded-xl"
    >
    ...
    {/* Left sidebar notches (outside motion div) */}
    {!isVisible && (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.6 }}
        transition={{ delay: 0.3, duration: 0.5 }}
        className="absolute top-1/2 -translate-y-1/2 left-0 flex flex-col gap-3 pointer-events-none z-[251]"
        style={{ left: '-10px' }}
      >
        {[0, 1, 2].map((i) => (
          <div
