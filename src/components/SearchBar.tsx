...
/* ═══════════════════════════════════════════════════════════════
   OPENSPY — Enhanced Search / Locate Bar
...
function getResultIcon(type: string, category: string) {
  ...
  if (['road', 'street', 'highway', 'path'].includes(type) || category === 'highway') {
    return <Navigation className="w-3 h-3 text-[#E0E0E0] flex-shrink-0" />;
  }
  ...
  if (['city', 'town', 'village', 'municipality'].includes(type)) {
    return <Landmark className="w-3 h-3 text-[#D0D0D0] flex-shrink-0" />;
  }
    ...
    timerRef.current = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(
          ...
          { headers: { 'Accept-Language': 'en', 'User-Agent': 'OpenSpy-Intelligence-Platform/1.0' } }
        );
  ...
  if (!open && !alwaysExpanded) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 glass-panel-sm px-3 py-2 text-[9px] font-mono tracking-[0.15em] text-[var(--text-muted)] hover:text-[var(--gold-primary)] hover:border-[var(--border-active)] transition-all hover:shadow-[0_0_12px_rgba(255,255,255,0.08)]"
      >
