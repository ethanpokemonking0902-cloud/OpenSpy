...
const TABS = [
  { id: 'scanner', label: 'PORT SCAN', icon: Radar, placeholder: 'IP or hostname', color: '#FFFFFF' },
  { id: 'vuln', label: 'VULN SWEEP', icon: Bug, placeholder: 'IP or hostname', color: '#FF3D3D' },

  { id: 'dns', label: 'DNS', icon: Server, placeholder: 'Domain name', color: '#FFFFFF' },
  { id: 'whois', label: 'WHOIS', icon: FileText, placeholder: 'Domain name', color: '#E0E0E0' },
  { id: 'certs', label: 'CERTS', icon: Lock, placeholder: 'Domain name', color: '#E040FB' },
  { id: 'threats', label: 'THREATS', icon: AlertTriangle, placeholder: 'IP, domain, or hash', color: '#D0D0D0' },
  { id: 'headers', label: 'HEADERS', icon: Code, placeholder: 'URL to inspect', color: '#FFFFFF' },
  { id: 'ssl', label: 'SSL/TLS', icon: Shield, placeholder: 'Domain name', color: '#E0E0E0' },
...
function OsintPanelInner({ isMobile, onSweepVisualize, onScanGeolocate }: OsintPanelProps) {
  ...
  const renderStructuredResults = () => {
    ...
    if (activeTab === 'scanner') {
      ...
      return (
        <div>
          <SectionHeader title="HOST INFO" icon={Server} color="#FFFFFF" />
          <ResultRow label="Target" value={host} color="#FFFFFF" />
          ...
          {Array.isArray(ports) && ports.length > 0 && (
            <>
              <SectionHeader title={`OPEN PORTS (${ports.length})`} icon={Wifi} color="#E0E0E0" />
    ...
    if (activeTab === 'vuln') {
      ...
      return (
        <div>
          ...
          <ResultRow label="Total CVEs" value={Array.isArray(vulns) ? vulns.length : 0} color={Array.isArray(vulns) && vulns.length > 0 ? '#FF3D3D' : '#E0E0E0'} />
          ...
          {exploits.length > 0 && (
            <div className="mt-4">
              <SectionHeader title={`POSSIBLE EXPLOITS (${exploits.length})`} icon={AlertTriangle} color="#D0D0D0" />
              <div className="mt-2 space-y-1">
    ...
    if (activeTab === 'dns') {
      return (
        <div>
          <SectionHeader title="DNS RECORDS" icon={Server} color="#FFFFFF" />
          <ResultRow label="Domain" value={r.domain || query} color="#FFFFFF" />
    ...
    if (activeTab === 'whois') {
      return (
        <div>
          <SectionHeader title="WHOIS INTELLIGENCE" icon={FileText} color="#E0E0E0" />
          <SanctionsBadge match={r.sanctions_match} />
          <ResultRow label="Domain" value={r.domain_name || r.domainName || query} color="#E0E0E0" />
    ...
    if (activeTab === 'shodan') {
      return (
        <div>
          ...
          {r.ports?.length > 0 && <ResultRow label="Open Ports" value={r.ports.join(', ')} color="#FFFFFF" />}
          {r.tags?.length > 0 && <ResultRow label="Tags" value={r.tags.join(', ')} color="#D0D0D0" />}
          {r.vulns?.length > 0 && (
            <div className="mt-2 p-2 border border-red-500/30 bg-red-500/10 rounded">
              ...
              <div className="flex flex-wrap gap-1">
                {r.vulns.slice(0, 10).map((v: string) => (
                  <a key={v} href={`https://nvd.nist.gov/vuln/detail/${v}`} target="_blank" rel="noreferrer" className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-[#1A1A18] text-[#A0A0A0] hover:text-[#FF3D3D]">{v}</a>
                ))}
                {r.vulns.length > 10 && <span className="text-[9px] font-mono text-[#A0A0A0]">+{r.vulns.length - 10} more</span>}
              </div>
            </div>
          )}
    ...
    if (activeTab === 'bgp') {
      return (
        <div>
          <SectionHeader title="BGP ROUTING INTELLIGENCE" icon={Globe} color="#FFFFFF" />
          <ResultRow label="Query" value={r.query} color="#E0E0E0" />
          {r.type === 'ip' && r.ip && (
            <>
              {r.ip.prefixes?.map((p: any, i: number) => (
                <div key={i} className="mt-2 p-2 border border-[#E0E0E0]/20 bg-[#E0E0E0]/5 rounded">
                  <ResultRow label="ASN" value={`AS${p.asn.asn} - ${p.asn.name}`} color="#E0E0E0" />
          ...
          {r.type === 'asn' && r.asn && (
            <div className="mt-2 p-2 border border-[#E0E0E0]/20 bg-[#E0E0E0]/5 rounded">
              <ResultRow label="ASN" value={`AS${r.asn.asn}`} color="#E0E0E0" />
    ...
    if (activeTab === 'mac') {
      return (
        <div>
          <SectionHeader title="MAC VENDOR LOOKUP" icon={Fingerprint} color="#D0D0D0" />
          <ResultRow label="MAC Address" value={r.mac} color="#D0D0D0" />
          <ResultRow label="Vendor" value={r.vendor} color={r.vendor === 'Not Found' ? '#FF3D3D' : '#E0E0E0'} />
        </div>
      );
    }
    ...
    if (activeTab === 'github') {
      return (
        <div>
          <SectionHeader title="GITHUB RECON" icon={Terminal} color="#E0E0E0" />
          <div className="flex items-center gap-3 mb-2">
            {r.avatar_url && <img src={r.avatar_url} alt="avatar" className="w-10 h-10 rounded-full border border-[#E0E0E0]/30" />}
            <div>
              <div className="text-[12px] font-mono font-bold text-[#E0E0E0]">{r.name || r.username}</div>
    ...
    if (activeTab === 'threats') {
      return (
        <div>
          <SectionHeader title="THREAT INTELLIGENCE" icon={AlertTriangle} color="#D0D0D0" />
          <ResultRow label="Query" value={query} color="#D0D0D0" />
          <ResultRow label="Risk Score" value={r.risk_score || r.score} color={
            (r.risk_score || r.score || 0) > 70 ? '#FF3D3D' : (r.risk_score || r.score || 0) > 40 ? '#D0D0D0' : '#00E676'
          } />
    ...
    if (activeTab === 'ssl') {
      return (
        <div>
          <SectionHeader title="SSL/TLS ANALYSIS" icon={Shield} color="#E0E0E0" />
          <ResultRow label="Target" value={query} color="#E0E0E0" />
          <ResultRow label="Protocol" value={r.protocol || r.tls_version} />
          <ResultRow label="Cipher" value={r.cipher || r.cipher_suite} />
          <ResultRow label="Valid" value={r.valid !== undefined ? (r.valid ? 'YES' : 'NO') : undefined} color={r.valid ? '#E0E0E0' : '#FF3D3D'} />
      ...
      {sweepProgress && loading && (
        <div className="p-3 rounded-lg border border-[#FF3D3D]/30 bg-[#FF3D3D]/5">
          ...
          <div className="w-full h-1.5 bg-[#1A1A18] rounded-full overflow-hidden">
            <div className="h-full rounded-full" style={{ width: '100%', background: 'linear-gradient(90deg, #FF3D3D, #D0D0D0, #D0D0D0)', animation: 'sweep-pulse 1.5s ease-in-out infinite' }} />
          </div>
        </div>
      )}
                        ...
                        <div className="flex flex-col gap-2">
                          {device.vulns.map((cveId: string) => {
                            ...
                            const severityColor = !info?.severity ? '#5C5A54'
                              ...
                              : info.severity === 'HIGH' ? '#D0D0D0'
                              : info.severity === 'MEDIUM' ? '#D0D0D0'
                              : '#E0E0E0';
                            return (
      ...
      <div className="fixed top-4 bottom-4 right-4 w-[40vw] min-w-[600px] max-w-[800px] z-[999] glass-panel bg-[#0a0a09]/95 backdrop-blur-2xl border border-[var(--cyan-primary)]/40 rounded-xl flex flex-col overflow-hidden shadow-2xl shadow-[var(--cyan-primary)]/20 transition-all duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-secondary)] bg-[#111]">
          <div className="flex items-center gap-3">
            ...
            <span className="hud-text text-[16px] text-[var(--text-primary)]">OpenSpy RECON TOOLKIT</span>
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.6 }} className="glass-panel flex flex-col overflow-hidden pointer-events-auto shrink-0 h-[500px] max-h-[80vh] resize-y">
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)] hover:bg-[var(--hover-accent)] transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-1.5 h-1.5 rounded-full bg-[var(--cyan-primary)] animate-pulse" />
