...
import {
  ...
  Wifi, Globe, Target, Activity, Shield, QrCode
} from 'lucide-react';
...
function classify(name:string,appearance?:number,mfr?:string,svcs?:number[]):{type:string;icon:typeof Bluetooth;color:string;by:string}{
  if(appearance!=null){const c=(appearance>>6)&0x3FF;const s=appearance&0x3F;if(c===1)return{type:'Phone',icon:Smartphone,color:'#D0D0D0',by:'appearance'};if(c===2)return{type:'Computer',icon:Cpu,color:'#E0E0E0',by:'appearance'};if(c===3)return{type:'Watch',icon:Watch,color:'#E040FB',by:'appearance'};if(c===5)return{type:'Display',icon:Tv,color:'#FFFFFF',by:'appearance'};if(c===6)return{type:'Remote',icon:Radio,color:'#90A4AE',by:'appearance'};if(c===10)return{type:'Media Player',icon:Tv,color:'#FFFFFF',by:'appearance'};if(c===15){if(s===1)return{type:'Keyboard',icon:Keyboard,color:'#E0E0E0',by:'appearance'};if(s===2)return{type:'Mouse',icon:Mouse,color:'#D0D0D0',by:'appearance'};if(s===4)return{type:'Gamepad',icon:Gamepad2,color:'#FF6B6B',by:'appearance'};return{type:'HID',icon:Keyboard,color:'#E0E0E0',by:'appearance'};}if(c===33)return{type:'Speaker',icon:Speaker,color:'#D0D0D0',by:'appearance'};if(appearance>=0x0841&&appearance<=0x087F)return{type:'Headphones',icon:Headphones,color:'#B388FF',by:'appearance'};if(appearance===0x0840)return{type:'Speaker',icon:Speaker,color:'#D0D0D0',by:'appearance'};}
  if(svcs&&svcs.length>0){if(svcs.includes(0x1848)||svcs.includes(0x1844))return{type:'Audio',icon:Speaker,color:'#D0D0D0',by:'service'};if(svcs.includes(0x1812))return{type:'HID',icon:Keyboard,color:'#E0E0E0',by:'service'};if(svcs.includes(0x180D))return{type:'Heart Rate',icon:Watch,color:'#E040FB',by:'service'};if(svcs.includes(0x181A))return{type:'Env Sensor',icon:Fan,color:'#D0D0D0',by:'service'};}
  if(mfr){if(/bose|jbl|sonos|harman|marshall|yamaha|denon|b&o/i.test(mfr))return{type:'Speaker',icon:Speaker,color:'#D0D0D0',by:'manufacturer'};if(/logitech|corsair|razer|steelseries/i.test(mfr))return{type:'Peripheral',icon:Mouse,color:'#D0D0D0',by:'manufacturer'};if(/fitbit|garmin|polar|suunto|coros/i.test(mfr))return{type:'Wearable',icon:Watch,color:'#E040FB',by:'manufacturer'};}
  const l=name.toLowerCase();if(/\btv\b|bravia|roku|chromecast|fire.?stick|apple.?tv/i.test(l))return{type:'Television',icon:Tv,color:'#FFFFFF',by:'name'};if(/speaker|soundbar|bose|jbl|sonos|echo|homepod/i.test(l))return{type:'Speaker',icon:Speaker,color:'#D0D0D0',by:'name'};if(/headphone|airpod|buds|earbud|wh-1000|wf-1000|qc|jabra|galaxy.?buds/i.test(l))return{type:'Headphones',icon:Headphones,color:'#B388FF',by:'name'};if(/watch|band|fitbit|garmin|amazfit|polar|mi.?band/i.test(l))return{type:'Wearable',icon:Watch,color:'#E040FB',by:'name'};if(/gamepad|controller|xbox|playstation|dualsense|joy.?con/i.test(l))return{type:'Controller',icon:Gamepad2,color:'#FF6B6B',by:'name'};if(/keyboard|keychron/i.test(l))return{type:'Keyboard',icon:Keyboard,color:'#E0E0E0',by:'name'};if(/mouse|mx.?master|trackpad/i.test(l))return{type:'Mouse',icon:Mouse,color:'#D0D0D0',by:'name'};if(/phone|iphone|galaxy.?[saz]|pixel/i.test(l))return{type:'Phone',icon:Smartphone,color:'#D0D0D0',by:'name'};if(/bulb|light|hue|lifx|nanoleaf|govee/i.test(l))return{type:'Light',icon:Lightbulb,color:'#FFFFFF',by:'name'};if(/printer|epson|canon|brother/i.test(l))return{type:'Printer',icon:Printer,color:'#90A4AE',by:'name'};if(/tile|chipolo|tracker|airtag|smarttag/i.test(l))return{type:'Tracker',icon:Radio,color:'#90A4AE',by:'name'};if(/lock|schlage|august|yale/i.test(l))return{type:'Smart Lock',icon:Plug,color:'#FF6B6B',by:'name'};if(/scale|withings|renpho/i.test(l))return{type:'Scale',icon:Watch,color:'#E040FB',by:'name'};
...
const OP_CLR:Record<string,string>={SCAN:'#E0E0E0',PROBE:'#B388FF',READ:'#E0E0E0',WRITE:'#D0D0D0',NOTIFY:'#D0D0D0',CONNECT:'#E0E0E0',DISCONNECT:'#FF6B6B',ENUM:'#D0D0D0',ERROR:'#FF3D3D',SUB:'#E040FB',INFO:'#90A4AE',GPS:'#D0D0D0',NET:'#D0D0D0',RSSI:'#E040FB'};
...
export default function WorldRemote({onClose,onPlaceOnMap}:{onClose?:()=>void,onPlaceOnMap?:(devs:{id:string,name:string,lat:number,lng:number,type:string,color:string}[])=>void}){
  ...
  const generateQRCode=useCallback((text:string):string=>{
    // Generate QR code URL using qrserver API
    return `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(text)}&margin=2`;
  },[]);
  
  const getMobileLink=useCallback(():string=>{
    // Generate a link that opens the mobile Bluetooth scanning interface
    const baseUrl=typeof window!=='undefined'?`${window.location.protocol}//${window.location.host}`:'';
    return `${baseUrl}/bluetooth-mobile`;
  },[]);
    ...
    <div className={`glass-panel flex flex-col overflow-hidden pointer-events-auto shrink-0 ${full?'w-full h-full':'h-[500px] max-h-[80vh] resize-y'}`}>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2.5 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)] hover:bg-[var(--hover-accent)] transition-colors">
        ...
        <div className="flex items-center gap-2">
          ...
          <motion.button whileTap={{scale:0.95}} onClick={()=>setShowQR(!showQR)} 
            className="px-2.5 py-1 rounded-md text-[8px] font-mono font-bold tracking-wider flex items-center gap-1.5"
            style={{background:'rgba(255,184,0,0.08)',color:'#FFB800',border:'1px solid rgba(255,184,0,0.1)'}}>
            <QrCode className="w-3 h-3"/>QR
          </motion.button>
          {geoCount>0&&<motion.button whileTap={{scale:0.95}} onClick={placeOnWorldMap}
            className="px-2.5 py-1 rounded-md text-[8px] font-mono font-bold tracking-wider flex items-center gap-1.5"
            style={{background:'rgba(208,208,208,0.08)',color:'#D0D0D0',border:'1px solid rgba(208,208,208,0.1)'}}>
                    ...
                    <div className="rounded-xl overflow-hidden transition-all" style={{background:dev.connected?`linear-gradient(135deg,${dev.color}06,${dev.color}02)`:'rgba(255,255,255,0.015)',border:`1px solid ${dev.connected?dev.color+'15':'rgba(255,255,255,0.04)'}`,boxShadow:dev.connected?`0 0 16px ${dev.color}08`:'none'}}>
                      ...
                      <button className="w-full px-3 py-2.5 flex items-center gap-2.5 text-left" onClick={()=>setExDev(isE?null:dev.id)}>
                        <motion.div animate={dev.probing?{rotate:[0,360]}:{}} transition={{duration:2,repeat:Infinity,ease:'linear'}} className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 relative" style={{background:`linear-gradient(135deg,${dev.color}12,${dev.color}05)`,border:`1px solid ${dev.color}18`}}>
                          ...
                          {dev.connected&&<div className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-[#E0E0E0] border animate-pulse" style={{borderColor:'#0D0D0C'}}/>}
                        </motion.div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            <span className="text-[10px] font-mono font-bold text-[var(--text-primary)] truncate">{dev.name}</span>
                            {dev.connected&&<span className="text-[6px] font-mono px-1 py-0.5 rounded bg-[#E0E0E0]/10 text-[#E0E0E0] tracking-widest font-bold">LIVE</span>}
                          </div>
                          {dev.probing?(
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {dev.battery!=null&&<span className="text-[7px] font-mono flex items-center gap-0.5" style={{color:dev.battery>50?'#E0E0E0':dev.battery>20?'#D0D0D0':'#FF3D3D'}}><BatteryMedium className="w-2.5 h-2.5"/>{dev.battery}%</span>}
                              {dev.geo&&<span className="text-[6px] font-mono text-[#D0D0D0] flex items-center gap-0.5"><MapPin className="w-2 h-2"/></span>}
                      ...
                      <AnimatePresence>{isE&&!dev.probing&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="px-3 pb-3" style={{borderTop:'1px solid rgba(255,255,255,0.03)'}}>
                        ...
                        <div className="mt-2 mb-2.5 rounded-lg p-2.5" style={{background:'rgba(0,0,0,0.25)',border:'1px solid rgba(255,255,255,0.03)'}}>
                          ...
                          <div className="space-y-1">
                            ...
                            {dev.geo&&<div className="flex gap-2"><span className="text-[7px] font-mono text-[var(--text-muted)] w-[65px] shrink-0 tracking-wider">GPS</span><span className="text-[8px] font-mono text-[#D0D0D0]">{dev.geo.lat.toFixed(5)}, {dev.geo.lng.toFixed(5)} ±{Math.round(dev.geo.acc)}m</span></div>}
                          ...
                          :gattSvcs.map(svc=>(<div key={svc.uuid} className="mb-0.5">
                            ...
                            <AnimatePresence>{exSvc===svc.uuid&&(<motion.div initial={{height:0}} animate={{height:'auto'}} exit={{height:0}} className="overflow-hidden"><div className="pl-3 pr-1 py-1 space-y-1">{svc.chars.map(ch=>{const ck=`${svc.uuid}/${ch.uuid}`;const fl=[ch.p.r&&'R',ch.p.w&&'W',ch.p.wn&&'Wn',ch.p.n&&'N',ch.p.i&&'I'].filter(Boolean).join('·');const wV=wIn[ck]||'';const wOk=!wV||validHex(wV);return(
                              <div key={ch.uuid} className="p-2 rounded-lg" style={{background:'rgba(0,0,0,0.25)',borderLeft:`2px solid ${dev.color}12`}}>
                                ...
                                {ch.value&&<div className="flex items-center gap-1 mb-1"><div className="flex-1 rounded px-1.5 py-1 min-w-0" style={{background:'rgba(0,0,0,0.3)'}}><div className={`text-[7px] font-mono truncate ${ch.notifying?'text-[#D0D0D0]':'text-[#E0E0E0]'}`}>{ch.value}</div>{ch.hex&&ch.hex!==ch.value&&<div className="text-[5px] font-mono text-[var(--text-muted)] truncate mt-0.5 opacity-40">{ch.hex}</div>}</div><button onClick={()=>{navigator.clipboard?.writeText(ch.value||'');setCopied(ck);setTimeout(()=>setCopied(null),1500);}} className="p-0.5 rounded hover:bg-white/5 shrink-0">{copied===ck?<Check className="w-2 h-2 text-[var(--alert-green)]"/>:<Copy className="w-2 h-2 text-[var(--text-muted)]"/>}</button></div>}
              ...
              <div className="flex-1 overflow-y-auto styled-scrollbar p-2.5 space-y-2.5">
                ...
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center justify-between" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                    <div className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-[#D0D0D0]"/><span className="text-[8px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">NETWORK RECON</span></div>
                    <motion.button whileTap={{scale:0.95}} onClick={runNetRecon} disabled={netLoading} className="px-2 py-0.5 rounded text-[7px] font-mono font-bold tracking-wider disabled:opacity-30" style={{background:'rgba(208,208,208,0.06)',color:'#D0D0D0',border:'1px solid rgba(208,208,208,0.08)'}}>{netLoading?'...':'PROBE'}</motion.button>
                  </div>
                  ...
                  {netIntel&&<div className="p-2.5 space-y-2">
                    {netIntel.localIPs.length>0&&<div><div className="text-[7px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">LOCAL IPs</div>{netIntel.localIPs.map((ip,i)=><div key={i} className="text-[9px] font-mono text-[#D0D0D0] pl-2">{ip}</div>)}</div>}
                    ...
                    {netIntel.openPorts.length>0&&<div><div className="text-[7px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">OPEN PORTS</div>{netIntel.openPorts.map((p,i)=><div key={i} className="flex items-center gap-2 pl-2 py-0.5"><span className="text-[8px] font-mono font-bold text-[#E0E0E0]">:{p.port}</span><span className="text-[7px] font-mono text-[var(--text-muted)]">{p.service}</span></div>)}</div>}
                  </div>}
                </div>
                {/* Export */}
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm rounded-xl z-50">
          <motion.div initial={{scale:0.9,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.9,opacity:0}} className="flex flex-col items-center gap-3 bg-black/70 p-6 rounded-lg border border-white/10">
            <div className="flex items-center justify-between w-full">
              ...
              <button onClick={()=>setShowQR(false)} className="text-white/50 hover:text-white/80"><X className="w-3 h-3"/></button>
            </div>
            
            <div className="bg-white p-2 rounded-lg">
              <img src={generateQRCode(getMobileLink())} alt="Scan for Bluetooth on mobile" className="w-[200px] h-[200px]" />
            </div>
            
            <div className="text-center">
              <p className="text-[9px] font-mono text-white/70 mb-1">Scan with your phone to access</p>
              <p className="text-[8px] font-mono text-[#FFB800] tracking-wider">Bluetooth Scanner</p>
            </div>
            
            <button onClick={()=>{navigator.clipboard?.writeText(getMobileLink()); setCopied('qr'); setTimeout(()=>setCopied(null), 1500);}} className="text-[8px] font-mono px-3 py-1.5 rounded bg-white/5 hover:bg-white/10 border border-white/10 text-white/70 hover:text-white transition-colors flex items-center gap-1.5">
              {copied==='qr'?<Check className="w-2.5 h-2.5"/>:<Copy className="w-2.5 h-2.5"/>}
