...
const CryptoIcon = ({ symbol }: { symbol: string }) => {
  if (symbol === 'BTC') return (
    <svg className="w-3.5 h-3.5" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M22.5 13.5c0-2-1.5-3-3.5-3h-1.5v-2h-2v2h-1.5v-2h-2v2h-2.5v2h1.5c.5 0 1 .5 1 1v6c0 .5-.5 1-1 1h-1.5v2h2.5v2h2v-2h1.5v2h2v-2c2 0 4-1 4-3 0-1.5-.5-2.5-1.5-3 1-.5 1.5-1.5 1.5-2.5zm-5 4c0 1-1 1-1.5 1h-2v-3h2c1 0 1.5 0 1.5 1v1zm-.5-4.5c0 1-1 1-1.5 1h-2v-2.5h2c.5 0 1.5 0 1.5 1v.5z" fill="#1A1A1A"/>
    </svg>
  );
  if (symbol === 'ETH') return (
    <svg className="w-3.5 h-3.5" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M15.9 2L7 16.8l8.9 5.3 8.9-5.3L15.9 2z" fill="#E0E0E0"/>
      <path d="M15.9 24v6.8l8.9-12.6-8.9 5.8z" fill="#E0E0E0"/>
      <path d="M7 18.2l8.9 12.6V24l-8.9-5.8z" fill="#E0E0E0"/>
    </svg>
  );
...
export default function GlobalStatusBar() {
    ...
    <>
      ...
      {quakes.map(quake => (
        ...
        >
          ...
          <span className="text-[#E0E0E0] font-bold tracking-wider">M{quake.magnitude.toFixed(1)}</span>
  ...
  return (
    <motion.div
      ...
      className="hidden md:block absolute bottom-0 left-1/2 -translate-x-1/2 z-[198] pointer-events-none w-full"
    >
      <div className="glass-panel rounded-none rounded-t-xl mx-0 px-4 py-2 flex items-center text-[8px] font-mono tracking-wider pointer-events-auto" style={{ backdropFilter: 'blur(10px)' }}>
        {/* Static label */}
        <div className="flex-shrink-0 flex items-center gap-1.5 pr-3 border-r border-[var(--cyan-primary)]/30">
          <span className="text-[var(--cyan-primary)] font-bold">LIVE</span>
        </div>

        {/* CSS-animated ticker */}
        <div className="flex-1 overflow-hidden relative ml-3" style={{ maskImage: 'linear-gradient(to right, transparent, black 5%, black 95%, transparent)' }}>
      ...
      {/* Hover tooltips */}
      {hoveredQuake && (
        <div className="absolute bottom-[56px] left-1/2 -translate-x-1/2 z-[1000] pointer-events-none">
          <div className="glass-panel rounded-xl px-4 py-3 text-[10px] font-mono whitespace-nowrap" style={{ borderColor: '#D0D0D040', backdropFilter: 'blur(10px)' }}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[12px] text-[#D0D0D0]">🌋</span>
              <span className="font-bold text-[#E0E0E0]">Magnitude {hoveredQuake.magnitude.toFixed(1)}</span>
