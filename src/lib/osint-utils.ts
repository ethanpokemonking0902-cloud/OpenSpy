...
export function classifyDevice(
  ...
  if (portSet.has(5060) || portSet.has(5061)) {
    return { device_type: 'VoIP/SIP', device_icon: 'Phone', device_color: '#E0E0E0' };
  }
  if (cpeLower.some((c) => /mikrotik|ubiquiti|cisco|juniper|fortinet/.test(c)) || portSet.has(161) || portSet.has(8291)) {
    return { device_type: 'Router/Switch', device_icon: 'Router', device_color: '#FFFFFF' };
  }
  ...
  if (portSet.has(53)) {
    return { device_type: 'DNS Server', device_icon: 'Server', device_color: '#D0D0D0' };
  }
  ...
  if (portSet.has(80) || portSet.has(443) || portSet.has(8080) || portSet.has(8443)) {
    return { device_type: 'Web Server', device_icon: 'Globe', device_color: '#E0E0E0' };
  }
