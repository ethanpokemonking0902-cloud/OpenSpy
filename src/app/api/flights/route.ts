...
async function getOpenSkyToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { console.warn('[OpenSpy] OpenSky token failed:', res.status); return null; }
    const data = await res.json();
    if (!data.access_token) {
      console.warn('[OpenSpy] OpenSky token response missing access_token');
      return null;
    }
    ...
    console.warn('[OpenSpy] OpenSky token error:', e);
    return null;
  }
}
...
export async function GET() {
  ...
  fetchPromise = (async () => {
    ...
    if (osRes.status === 'fulfilled') {
      if (osRes.value.status === 429) {
        ...
        console.warn('[OpenSpy] OpenSky 429 — cooling down 15 min');
        ...
        try {
          ...
          console.warn('[OpenSpy] OpenSky parse error:', e);
        }
      } else {
        await osRes.value.body?.cancel();
      }
    ...
    if (!openSkyWorked) {
      source = 'regional';
      console.warn('[OpenSpy] OpenSky unavailable — fanning out 30×2 regional queries');
  ...
  try {
    ...
    console.error('[OpenSpy] Flight fetch error:', error);
    fetchPromise = null;
    // Stale-cache fallback: return last known good data instead of blank map
    if (cachedData) {
      console.warn('[OpenSpy] Returning stale flight cache as fallback');
