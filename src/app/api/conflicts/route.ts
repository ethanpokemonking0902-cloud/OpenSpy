  ...
  try {
    ...
    const feedPromises = RSS_FEEDS.map(async (url) => {
      try {
        ...
        if (!res.ok) {
          console.log(`[OpenSpy] RSS Fetch Failed for ${url}: ${res.status}`);
          return [];
        }
        
        const xml = await res.text();
        const rawItems = xml.split(/<item>/i).slice(1);
        console.log(`[OpenSpy] RSS ${url} returned ${rawItems.length} items`);
        ...
        console.log(`[OpenSpy] RSS Fetch Error for ${url}:`, (err as Error).message);
        return [];
      }
    });
    ...
    console.log(`[OpenSpy] Mapped ${allEvents.length} conflict events from RSS.`);
  } catch (e) {
...
export async function GET() {
  try {
    ...
    console.error('[OpenSpy] Conflict API error:', error);
